...
test('renders properly', t => {
  ...
  t.is(wrapper.find('Header').prop('toggleAddKeeper'), wrapper.instance().toggleAddKeeperSection);
...
test('calling toggleAddKeeperSection dispatches toggleAddKeeper', t => {
  const wrapper = shallow(
    <App {...props} />
  );

  wrapper.instance().toggleAddKeeperSection();
  t.truthy(dispatch.calledOnce);
  t.truthy(dispatch.calledWith(toggleAddKeeper()));
});
